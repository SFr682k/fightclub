    ...
    QGroupBox(parent)
{
    ...
    timedisplay->setMinimumSize(QSize(86,24));
    timedisplay->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
    ...
    progresslayout->addWidget(timedisplay,0,1);
    */
...
void ClientBoxWidget::updatePhaseProgress() {
    ...
    if(progress <= 1.0) progressbar->setValue(((int) (progress*1000)) % 1000);
    else                progressbar->setValue(1000);
}
...
void ClientBoxWidget::setProgressBarHeight(int windowHeight) {
    progressbar->setMinimumHeight(windowHeight*0.01);
    progressbar->setMaximumHeight(windowHeight*0.01);
}
